...
/// @title Arena Survival Leaderboard V4
/// @notice On-chain leaderboard with PAY-BEFORE-PLAY mechanics
/// @dev Entry fee: 0.001 ETH paid BEFORE playing, score submitted after
///      Prevents users from only submitting high scores
contract ArenaLeaderboardV4 {
    ...
    // Active games (paid but not yet submitted)
    ...
    // Events
    event GameStarted(address indexed player, uint256 epochId, uint256 timestamp);
    event ScoreSubmitted(address indexed player, uint32 score, uint32 wave, uint32 kills, uint256 rank, uint256 epochId);
    ...
    /// @notice Start a game by paying entry fee (MUST call before playing)
    /// @dev Payment is collected here, score submitted later via submitScore()
    function startGame() external payable {
        require(msg.value >= ENTRY_FEE, "Pay 0.001 ETH to play");
        require(!activeGames[msg.sender].active, "Already have active game");
        ...
        emit GameStarted(msg.sender, currentEpochId, block.timestamp);
    }

    /// @notice Submit score after game ends (must have called startGame first)
    ...
    function submitScore(
        ...
        require(activeGames[msg.sender].active, "No active game - call startGame first");

        // Clear active game
        activeGames[msg.sender].active = false;

        // Check if epoch changed during gameplay
        if (block.timestamp >= currentEpochStart + EPOCH_DURATION) {
            _endCurrentEpoch();
            _startNewEpoch();
        }
        ...
        emit ScoreSubmitted(msg.sender, score, wave, kills, rank, currentEpochId);
    }

    /// @notice Check if player has an active (paid) game
    function hasActiveGame(address player) external view returns (bool) {
        return activeGames[player].active;
    }

    /// @notice Forfeit active game (no refund, but clears active status)
    function forfeitGame() external {
        require(activeGames[msg.sender].active, "No active game");
        activeGames[msg.sender].active = false;
    }
